...
for m in m_range:
    ...
    p_inlet_range.append(c1.p.val)

eta_s_range = np.array(eta_s_range) * 100

fig, ax = plt.subplots(2)

ax[0].plot(m_range, eta_s_range)
ax[1].plot(m_range, p_inlet_range)

ax[0].set_ylabel("Isentropic efficiency in %")
ax[1].set_ylabel("Pressure at turbine inlet in bar")
ax[1].set_xlabel("Turbine mass flow in kg/s")
