...
# this is formulated in a tricky way, but it makes residual calculation easy
...
while True:
    ...
    residual = np.array([
        volumetric_flow_residual(v_dot, x[0], x[1], x[2], fluid),
        variable_specified_residual(p_1, x[1]),
        variable_equality_residual(x[0], x[3]),
        temperature_residual(t_1, x[1], x[2], fluid),
        temperature_residual(t_2_target, x[4], x[5], fluid),
    ...
    jacobian[0, [0, 1, 2]] = volumetric_flow_jacobian(v_dot, x[0], x[1], x[2], fluid)
    jacobian[1, 1] = -1
    jacobian[2, [0, 3]] = [1, -1]
